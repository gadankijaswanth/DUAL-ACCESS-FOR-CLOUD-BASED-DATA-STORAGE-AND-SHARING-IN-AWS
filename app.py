...
db = pymysql.connect(host='localhost',user='root',password='',port=3307,db='Cloud')
cursor=db.cursor()
...
def authorizationkey(s1=0,s2=''):
    ...
    sender_address = 'gadankijaswanth@gmail.com'
    sender_pass = 'sumtbzihrcfyfrxo'
    receiver_address = s2
...
def generatekeytouser(s1=0,s2="",s3=0):
    ...
    session.sendmail(sender_address, receiver_address, text)
    session.quit()
    sql="update request set pkey=%s where id='%s' " %(str(otp),s1)
    cursor.execute(sql)
    db.commit()
...
@app.route("/generatekeytouserforauthorization/<s1>/<s2>")
def generatekeytouserforauthorization(s1=0,s2=""):
    Otp = str(randint(123456789, 980976509))
    sender_address = 'gadankijaswanth@gmail.com'
    sender_pass = 'sumtbzihrcfyfrxo'
    receiver_address = s2
    message = MIMEMultipart()
...
#         # Ensure you are opening the file in binary mode if it’s not a text file
#         with open(dd, "r") as f:
#             data = f.read()
...
#         print(f"Time taken for algorithm execution: {execution_time} seconds")  # Print duration to console
#         # sql="update filesupload set execution_time=%s where Sl_No='%s' " %(otp,s1)
#         # sql = "update  into filesupload (execution_time) values (%s)"
#         # values = (execution_time)
#         # cursor.execute(sql, values)
#         # db.commit()
#         return render_template("DataOwnersUploadFiles.html", msg="success", files=Files, time_taken=execution_time)
...
def DataOwnersUploadFiles():
    if request.method == 'POST':
        ...
        # Here you decide whether to use RSA or AES based on some condition
        if algorithm == '1':  # You need to replace `some_condition` with your actual condition
            # AES Encryption Logic
            curr = time.time() 
            print(curr)
            secret_key = b'secret_key_16bytes'
            data = data
            encrypteddata = encrypt(data,secret_key)
            decrypteddata = decrypt(encrypteddata,secret_key)
            newcurr = time.time()
            
            performance = newcurr - curr
            sql = "insert into filesupload (owneremail, FileName, Keywords, Files,execution_time) values (%s, %s, %s, %s, %s)"
            values = (session["nj"], n, Keywords, encrypteddata,performance)
            cursor.execute(sql, values)
            db.commit()

            return render_template("DataOwnersUploadFiles.html", msg="success", files=filedata)
        else:
            curr = time.time()
            print(curr)
            ...
            decrypt_data = decrypt_message(encrypted_data,private_key)
            ...
            sql = "insert into filesupload (owneremail, FileName, Keywords, Files,execution_time) values (%s, %s, %s, %s,%s)"
            values = (session["nj"], n, Keywords, encrypted_data,performance)
            cursor.execute(sql, values)
            db.commit()
            return render_template("DataOwnersUploadFiles.html", msg="success", files=filedata)
    return render_template("DataOwnersUploadFiles.html")
...
def download(s1=0):
    sql="SELECT  FileName,AES_DECRYPT(Files, 'rupesh') FROM filesupload where Sl_No='%s'"% (s1)
    
    cursor.execute(sql)
    data = cursor.fetchall()
    filename=data[0][0]
    print(filename)
    f =open("uploadfiles/"+filename,'r')
    data = f.read()
    return render_template("download.html",row_val=data)
...
def key():
    if request.method=='POST':
        ...
        print(file,gkey,fid)
        sql="select FileName from filesupload,request where request.pkey='"+file+"' and request.id='"+gkey+"' and filesupload.Sl_No='"+fid+"' "
        cursor.execute(sql)
        data = cursor.fetchall()
        filename = data[0][0]
        print(filename)
        filepath = f"uploadFiles/{filename}"
